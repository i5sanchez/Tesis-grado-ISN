# -*- coding: utf-8 -*-
"""
Created on Thu May 16 18:47:24 2024
...
cable_1_5mm=[13.3*(1+1.7e-5*70),21*0.9,21.36,1.5,0.2] #[0] es la R en ohm/km a 90º, [1] es la Iz con los cables al sol, [2] caída de tensión en V/(A*km),[3] Sección del cable, [4] USD/metro de cable
...
def caidas_tension(I, V, cable, long_cables):
    ...
    Rcab=caida_voltios/I
    perd_pot=I**2*(Rcab)
    perd_pot_porc = round(perd_pot/(V*I)*100,2)
    Icc = round(V/Rcab,2)
    return caida_voltios, caida_porc, precio, validez_caida, validez_term,perd_pot,perd_pot_porc,Icc

def calc_prec(V, long_cables, cable, Idc):
    caida_voltios, caida_porc, precio_cab, validez_caida, validez_term, perd_pot, perd_pot_porc, Icc = caidas_tension(calc_corr_AC(12, Idc, 0.8, V), V, cable, long_cables)
    precio_dc_ac12_230 = 62.05
    precio_diodos_schottky = 14.7 * 8
    termomag = 6.86 * 2
    contador = 35.92 * 2
    ac_dc = 21.23 * 4
    ...
    if V == 230:
        precio += precio_dc_ac12_230 * 2
    elif V == 460:
        precio += precio_dc_ac12_230 * 2 + trafo230_460 * 2
    elif V == 690:
        precio += precio_dc_ac12_230 * 2 + trafo230_690 * 2
    elif V==920:
        precio += precio_dc_ac12_230*2+trafo230_920*2
    if validez_caida==True and validez_term==True:
        vale = 'Sí'
    else:
        vale='No'
    print(f'Para el caso de 12 V a {V} V y una distancia de {long_cables // 2} metros, la caída porcentual de tensión es del {caida_porc:.2f} % y una caída de potencia del {perd_pot_porc} %, con una Icc de {Icc} A y valdrían los cables y convertidores un total de {precio:.2f} USD. {vale} es válido')
...
def calc_prec_(V, long_cables, cable, Idc):
    ...
    precios_dc_ac = {
        230: precio_dc_ac12_230 * 2,
        460: precio_dc_ac12_230 * 2 + trafo * 2,
        690: precio_dc_ac12_230 * 2 + trafo * 2, 920: precio_dc_ac12_230*2+trafo*2
    }
...
calc_prec(230, long_cables, cable_16mm, 40)
...
plt.ylim(500, max(precios_optimos) + 500)  # Límites en el eje y, ajustado dinámicamente
...
plt.yticks(np.arange(500, max(precios_optimos) + 500, 20))  # Intervalo de 2500 en 2500 USD en el eje y
