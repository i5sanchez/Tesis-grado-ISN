...
cable_1_5mm=[13.7*(1+1.7e-5*70),30*0.9,1.5,0.2] #[0] es la R en ohm/km a 90º, [1] es la Iz con los cables al sol, [3] USD/metro de cable
...
def calc_prec(V,long_cables,cable):
    ...
    precio = precio_diodos_schottky+diferenciales+contactores+termomag+contador
    if V==12:
        precio+=precio_cab
    elif V==24:
...
def calc_prec_(V, long_cables, cable):
    ...
    precio = precio_cab + precios_dc_dc[V] + precio_diodos_schottky+diferenciales+contactores+termomag+contador
    NPC = precio*1.1 + (precio_diodos_schottky*2 + precios_dc_dc[V]*4 + diferenciales*(2/3) + termomag*(2/3) +contador*3)*1.1
...
calc_prec(1296,long_cables,cable_16mm)
...
for V in range(48, ultimo_nivel_tension + 1, 48):
    ...
    for distancia in range(10, 1501, 10):
        ...
        for cable_info in cables:
            ...
            precio, _, validez_term, validez_caida, caida_porc,NPC = calc_prec_(V, distancia*2, cable_info)
            
            # Almacenar los resultados en un vector
            vector_cable = {
                'seccion': cable_info[2],
                'precio': precio,
...
plt.ylim(0, 107000)  # Límites en el eje y desde 0 hasta 50000
...
plt.xticks(np.arange(0, 1500, 10))  # Intervalo de 100 en 100 metros en el eje x
plt.yticks(np.arange(0, 107001, 1000))  # Intervalo de 2500 en 2500 USD en el eje y
